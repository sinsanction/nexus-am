...
void bench_conv16_run() {
  ...
  for (k=1; k<=5; k++) {
    ...
    for (int i=0; i<m; i++) {
      ...
      uint32_t tmp_conv = Conv(k);
      B[0 * m + i] = ((tmp_conv / (k * k)) > 0xffff) ? 0xffff : (tmp_conv / (k * k));
      ...
      int32_t tmp_res2 = tmp_res / (k * k);
      C[0 * m + i] = (tmp_res2 < 0) ? 0 : (tmp_res2 > 0xffff) ? 0xffff : tmp_res2;

      if (B[0 * m + i] != C[0 * m + i]) {
        printf("  conv error: i=%d, j=0, conv_res=%d, std_res=%d, tmp_conv=%d, tmp_res=%d\n", i, B[0 * m + i], C[0 * m + i], tmp_conv, tmp_res);
        for (int si=0; si<k; si++) {
          for (int sj=0; sj<k; sj++) {
        ...
        printf("  ok: i=%d, j=0, std_res=%d, tmp_conv=%d\n", i, B[0 * m + i], tmp_conv);
      }

      for (int j=1; j<m; j++) {
        LoadV_P((uint64_t)(col_ptr), k, 0);
        uint32_t tmp_conv = Conv(k);
        B[j * m + i] = ((tmp_conv / (k * k)) > 0xffff) ? 0xffff : (tmp_conv / (k * k));
        ...
        int32_t tmp_res2 = tmp_res / (k * k);
        C[j * m + i] = (tmp_res2 < 0) ? 0 : (tmp_res2 > 0xffff) ? 0xffff : tmp_res2;

        if (B[j * m + i] != C[j * m + i]) {
          printf("  conv error: i=%d, j=%d, conv_res=%d, std_res=%d, tmp_conv=%d, tmp_res=%d\n", i, j, B[j * m + i], C[j * m + i], tmp_conv, tmp_res);
          for (int si=0; si<k; si++) {
            for (int sj=0; sj<k; sj++) {
          ...
          printf("  ok: i=%d, j=%d, std_res=%d, tmp_conv=%d\n", i, j, B[j * m + i], tmp_conv);
        }
