...
void bench_conv8_run() {
  ...
  for (k=1; k<=5; k++) {
    ...
    for (int i=0; i<m; i++) {
      ...
      B[0 * m + i] = ((tmp_conv / (k * k)) > 0xff) ? 0xff : (tmp_conv / (k * k));
      ...
      C[0 * m + i] = (tmp_res2 < 0) ? 0 : (tmp_res2 > 0xff) ? 0xff : tmp_res2;
      ...
      for (int j=1; j<m; j++) {
        ...
        uint32_t tmp_conv = Conv(k);
        B[j * m + i] = ((tmp_conv / (k * k)) > 0xff) ? 0xff : (tmp_conv / (k * k));
        ...
        int32_t tmp_res2 = tmp_res / (k * k);
        C[j * m + i] = (tmp_res2 < 0) ? 0 : (tmp_res2 > 0xff) ? 0xff : tmp_res2;
