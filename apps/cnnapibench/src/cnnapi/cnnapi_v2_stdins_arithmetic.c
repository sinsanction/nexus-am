...
//conv
image_mp_t *StdIns_Convolution_MP_SC(image_mp_t *input_image, kernel_mp_t *input_kernel, int strides) {
...
//multi channel
image_mp_mc_t *StdIns_Convolution_MP(image_mp_mc_t *input_image, kernel_mp_mc_t *input_kernel, int strides) {
    ...
    for (int i=0; i<img_mc->channel; i++) {
        for (int j=0; j<input_image->channel; j++) {
            ...
            img_tmp[j] = StdIns_Convolution_MP_SC(input_image->img[j], curr_kernel, strides);
        }
